defmodule BlockScoutWeb.API.V2.MudController do
  ...
  def world_table_records(conn, %{"world" => world_param, "table_id" => table_id_param} = params) do
    with {:ok, world} <- Hash.Address.cast(world_param),
      ...
      conn
      |> put_status(200)
      |> render(:records, %{records: records, table_id: table_id, schema: schema, next_page_params: next_page_params})
    end
  end
  ...
  def world_table_record(
    ...
    with {:ok, world} <- Hash.Address.cast(world_param),
      ...
      conn
      |> put_status(200)
      |> render(:record, %{record: record, table_id: table_id, schema: schema})
    end
  end
